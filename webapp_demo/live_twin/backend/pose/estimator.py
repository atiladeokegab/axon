...
from live_twin.backend.pose.model_asset import MODEL_PATH as _MODEL_PATH
from live_twin.backend.pose.model_asset import ensure_pose_model

DEFAULT_MODEL_PATH = str(_MODEL_PATH)
...
class ArmPoseEstimator:
    def __init__(self, model_path: str = DEFAULT_MODEL_PATH,
                 ...
                 side: str | None = None):
        # Make sure the model file is actually there BEFORE handing the path to
        # MediaPipe. Without this the failure surfaces as a FileNotFoundError
        # thrown from inside a C dispatcher several frames down, at the moment a
        # client connects rather than at startup - so the logs cheerfully report
        # "camera opened" while every request returns 503.
        ensure_pose_model()
